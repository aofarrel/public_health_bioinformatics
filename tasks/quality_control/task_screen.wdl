    ...
    if [[ "~{skip_screen}" = "false" ]] ; then
      ...
      if [ "${flag}" = "PASS" ]; then
        ...
        if [ "${estimated_genome_size}" -ge "~{max_genome_size}" ] ; then
          flag="FAIL; the genome size is estimated (${estimated_genome_size}) to be larger than the maximum of ~{max_genome_size} bps"
  ...
  command <<<
    ...
    if [[ "~{skip_screen}" = "false" ]] ; then
      ...
      if [ "${flag}" = "PASS" ]; then
        ...
        if [ "${estimated_genome_size}" -ge "~{max_genome_size}" ] ; then
          flag="FAIL; the genome size (${estimated_genome_size}) is estimated to be larger than the maximum of ~{max_genome_size} bps"
        elif [ "${estimated_genome_size}" -le "~{min_genome_size}" ] ; then
